...
class TSNDataSet(data.Dataset):
    ...
    def my_sample_indiecs(self,record):
        if record.num_frames > 24:
            start = np.random.randint(record.num_frames-24)
            return np.array(range(start,start+24))
        elif record.num_frames == 24:
            return np.array(range(24))
        else:
            miss = 24 - record.num_frames
            append = np.array([record.num_frames-1]*miss)
            origin = np.array(range(record.num_frames))
            return np.append(origin, append)
    ...
    def __getitem__(self, index):
        ...
        if not self.test_mode:
            if self.modality == 'Flow':
                ...
                # segment_indices = self.my_sample_indiecs(record)
